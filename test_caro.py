...
for v in viewpoints:
    ...
    for point in raster_points:
        ...
        if npvs[row , col] == 5: 
            ...
            # (vrow, vcol) is index of viewpoint. point is also the index of the given cell
...
for j in range(len(viewpoints)):
    ...
    for line in nested_line_lst:
        tan_old = -99
        for pixel in line[1:]:
            tan_new = slope(viewpoints[j], pixel, npi) # viewpoint is in coordinates. pixel in index
            col = pixel[1]
            row = pixel[0]
            if tan_new >= tan_old and npvs[row , col] != 2 and npvs[row , col] != 1:    #!=1 because we dont want to overwrite with second circle
                npvs[row , col] = 1
                tan_old = tan_new
